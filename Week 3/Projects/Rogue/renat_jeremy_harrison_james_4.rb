...
puts "How many rows do you want?"
    startingRow = gets.to_i
puts "How many columns do you want?"
    startingColumn = gets.to_i
grid = Array.new(startingRow) {Array.new(startingColumn) { 0 } }
...
playerrow = rand(0...startingRow)
playercolumn = rand(0...startingColumn) 
money = 0
#Money and walls and stuff
grid [rand(0...startingRow)][rand(0...startingColumn)] = "$"
grid [rand(0...startingRow)][rand(0...startingColumn)] = "$"
grid [rand(0...startingRow)][rand(0...startingColumn)] = "|"
grid [rand(0...startingRow)][rand(0...startingColumn)] = "|"
...
while on
    ...
    (0...startingRow).each do |row|
        (0...startingColumn).each do |column|
    ...
    if letter == "a"
        if grid[playerrow][playercolumn-1] == "|"
            ...
            playercolumn = playercolumn - 1
            if playercolumn < 0
                playercolumn = startingColumn - 1
            end
        end
    end

    if letter == "s"
        if playerrow+1 >= startingRow
                playerrow = 0
                grid[startingRow-1][playercolumn] = 0
        elsif grid[playerrow+1][playercolumn] == "|"
        elsif grid[playerrow+1][playercolumn] == "$"
            money = money + 10
            grid[playerrow][playercolumn] = 0
    ...
    if letter == "d"
        if grid[playerrow][playercolumn+1] == "|"
            ...
            grid[playerrow][playercolumn] = 0
            playercolumn = playercolumn + 1
            if playercolumn > startingColumn - 1
                playercolumn = 0
            end
        end
    end
