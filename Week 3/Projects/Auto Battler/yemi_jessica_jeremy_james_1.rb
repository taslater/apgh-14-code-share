...
if mainMenu == "3" # initiates fight menu
    ...
    if wantToFight == "1" #if the player wants to fight
        ...
        #if enemy["hp"]["healthPoints"] <= 0 # enemy's HP hits 0
        #    fightloop = false
        #    puts "Congratulations! You win!" # fight ends
        #    puts "You won 10 gold!"
        #    character["gold"]["characterGold"] = character["gold"]["characterGold"] + 10 # character recieves gold
        #elsif character["hp"]["healthPoints"] <= 0 # character's HP hits 0
        #    fightloop = false
        #    puts "You lost" # flavor text
        #    runGame = false # game shuts down
        #end # end fight loop
        if enemy ["hp"]["healthPoints"] <= 0 || character["hp"]["healthPoints"] <=0
            fightloop = false
            if enemy["hp"]["healthPoints"] <= 0
                puts "Congratulations! You win!" 
                puts "You won 10 gold!"
                character["gold"]["characterGold"] = character["gold"]["characterGold"] + 10 
            elsif character["hp"]["healthPoints"] <= 0 
                puts "You lost"
                runGame = false
            end
        end
